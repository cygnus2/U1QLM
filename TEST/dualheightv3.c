...
//  for(p=VOL2;p<VOL;p++) ising[p]=0;
  ...
  /* do this only for the 4x4 lattice */
  /* start with a conf with +2Q at (0,0) and -2Q at (2,2) at each time slice */
  for(t=0;t<LT;t=t+2){
    /* even time-slices */
    p=convert(0,0,t); ising[p]=1; 
    p=convert(2,0,t); ising[p]=1; 
    p=convert(1,1,t); ising[p]=1; 
    p=convert(3,1,t); ising[p]=1; 
    p=convert(0,2,t); ising[p]=1; 
    p=convert(2,2,t); ising[p]=-1; 
    p=convert(1,3,t); ising[p]=1; 
    p=convert(3,3,t); ising[p]=-1; 
    /* odd  time-slices */
    p=convert(1,0,t+1); ising[p]=-1; 
    p=convert(3,0,t+1); ising[p]=-1; 
    p=convert(0,1,t+1); ising[p]=-1; 
    p=convert(2,1,t+1); ising[p]=-1; 
    p=convert(1,2,t+1); ising[p]=-1; 
    p=convert(3,2,t+1); ising[p]=1; 
    p=convert(0,3,t+1); ising[p]=-1; 
    p=convert(2,3,t+1); ising[p]=-1; 
  }
...
void chkconf(){
  ...
  for(p=VOL2;p<VOL;p++){
    //if((ixc[p-VOL2]==0)&&(iyc[p-VOL2]==0)) continue;
    //if((ixc[p-VOL2]==2)&&(iyc[p-VOL2]==2)) continue;
...
void measureflux(int nspin){
  ...
  */
  ...
  for(p=0;p<nspin;p++){
     t=itc[list[p]];  
     q=iyc[list[p]]*LX + ixc[list[p]];
     sp1=ising[list[p]]; 
     //printf("%d %d %d %d %d\n",nspin,list[p],itc[list[p]],iyc[list[p]],ixc[list[p]]);
     /* forward timeslice */
     /* link-1 */ 
     p1=neigh[3][list[p]]; p2=neigh[5][p1];  
     sp2=ising[p2];
     if(sp1==sp2) fy[t][q]=-1; else fy[t][q]=1;
     /* link-2 */
     p1=neigh[4][list[p]]; p2=neigh[5][p1];
     sp2=ising[p2];
     if(sp1==sp2) fx[t][q]=-1; else fx[t][q]=1;
     /* link-3 */
     p1=neigh[1][list[p]]; p2=neigh[5][p1];
     sp2=ising[p2];
     if(sp1==sp2) fy[t][next[DIM+1][q]]=-1; else fy[t][next[DIM+1][q]]=1;
     /* link-4 */
     p1=neigh[2][list[p]]; p2=neigh[5][p1];
     sp2=ising[p2];
     if(sp1==sp2) fx[t][next[DIM+2][q]]=-1; else fx[t][next[DIM+2][q]]=1;
     /* backward timeslice */
     /* link-1 */ 
     p1=neigh[3][list[p]]; p2=neigh[0][p1];  
     sp2=ising[p2];
     if(sp1==sp2) fy[t][q]=-1; else fy[backt[t]][q]=1;
     /* link-2 */
     p1=neigh[4][list[p]]; p2=neigh[0][p1];
     sp2=ising[p2];
     if(sp1==sp2) fx[t][q]=-1; else fx[backt[t]][q]=1;
     /* link-3 */
     p1=neigh[1][list[p]]; p2=neigh[0][p1];
     sp2=ising[p2];
     if(sp1==sp2) fy[t][next[DIM+1][q]]=-1; else fy[backt[t]][next[DIM+1][q]]=1;
     /* link-4 */
     p1=neigh[2][list[p]]; p2=neigh[0][p1];
     sp2=ising[p2];
     if(sp1==sp2) fx[t][next[DIM+2][0]]=-1; else fx[backt[t]][next[DIM+2][q]]=1;

  }

  /* Measure charge and Ex and Ey (Wx and Wy) */
  for(t=0;t<LT;t++){
   flx=fly=0.0;
   for(p=0;p<SPV;p++){
    flx += fx[t][p];
    fly += fy[t][p];
    //Q = fx[t][p] - fx[t][next[DIM-1][p]] + fy[t][p] - fy[t][next[DIM-2][p]];
    //if(Q!=0) printf("%d %d %d\n",t,Q,p);
    /* average over flux configurations */
    //avflx[p] += fx[t][p];
    //avfly[p] += fy[t][p];
   }
   flx /= (2.0*LX);
